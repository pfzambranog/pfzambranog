...
Begin
   Execute dbo.Spp_notificaMantenimientoIndices @PnIdProceso    = @PnIdProceso,
...
Create Or Alter Procedure dbo.Spp_notificaMantenimientoIndices
  (@PnIdProceso             Integer,
...
--
   ...
   Create Table #tempAnalisisIndices
   (secuencia         Integer         Not Null Identity (1, 1) Primary Key,
...
--
   ...
   Begin Try
      Select @w_grupoCorreo     = dbo.Fn_splitStringColumna(@w_parametros, '|',  1),
             ...
             @w_ambiente        = dbo.Fn_splitStringColumna(@w_parametros, '|',  4),
             @w_tablaLog        = dbo.Fn_splitStringColumna(@w_parametros, '|',  5),
...
--
   ...
   Insert Into #tempAnalisisIndices
   (servidor,      basedatos, fechaProceso,
    error,         mensajeError)
   Select servidor,      basedatos, fechaProceso,
          error,         mensajeError
   ...
   Select  @w_fechaTermino = Max(fechaProceso)
   From    #tempAnalisisIndices;

--
-- Construcción del correo
--

   Select @w_cuerpo = Replace(@w_cuerpo, '&saludo&',        Trim(@w_saludo)),
          ...
          @w_cuerpo = Replace(@w_cuerpo, '&fechaproceso&',  Convert(Char(10), @w_fechaTermino, 103));


   Set @w_sql = Concat('Select Top 10 servidor,      basedatos "Base de Datos", ',
                               'error "Código Error",  mensajeError "Mensaje Error" ',
                       'From   #tempAnalisisIndices ',
                       'Order by secuencia ')
...
Declare
   @w_valor          Varchar(1500) = 'Procedimiento que notifica la Ejecución del Mantenimiento de Índices.',
   @w_procedimiento  Varchar( 100) = 'Spp_notificaMantenimientoIndices'
