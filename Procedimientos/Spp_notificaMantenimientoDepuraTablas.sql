...
Create Or Alter Procedure Spp_notificaMantenimientoDepuraTablas
  (@PnIdProceso                Integer,
...
--
   ...
   Set @w_sql = Concat('Select idProceso, secuencia, servidor, baseDatos, ',
                              'tabla,     registros ',
                       'From  ', @w_tablaLog, ' ',
...
--
   ...
   Select @w_cuerpo = Replace(@w_cuerpo, '&saludos&',       Trim(@w_saludo)),
          @w_cuerpo = Replace(@w_cuerpo, '&incidencia%',    Concat('<b>', @w_asunto,      '</b>')),
          ...
          @w_cuerpo = Replace(@w_cuerpo, '&t_tabla&',       Concat('<b>', @w_tablalOG,    '</b>'));

--
-- Construcción del correo
--

   Set @w_sql = Concat('Select top 20 Cast(tabla As Char(100)) "Tabla Depurada", ',
                              Concat('Format(registros, ', @w_comilla, '     ###,###,##', @w_comilla,') "Registros Eliminados" '),
                'From   #tempObjetos
                 Order by 1 ')
...
--
   ...
   Select  @w_html = Replace(@w_cuerpo, '&detalle&', @w_html);
   ...
   Begin
      ...
      If @w_idCorreo != 0
         Begin
            ...
            Set @w_sql = Concat('Update dbo.', @w_tablalog, ' ',
                                'Set    informado    = 2, ',
...
Declare
   @w_valor          Nvarchar(250) = 'Genera Notifición del Proceso de Depuración de Datos.',
   @w_procedimiento  NVarchar(250) = 'Spp_notificaMantenimientoDepuraTablas';
