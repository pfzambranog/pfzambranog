...
Create Or Alter Procedure dbo.Spp_reportaIncTiempoProc
...
--
   ...
   Select @w_grupoCorreo     = dbo.Fn_splitStringColumna(@w_parametros, '|', 1),
          ...
          @w_objeto          = dbo.Fn_splitStringColumna(@w_parametros, '|', 10),
          @w_query           = dbo.Fn_splitStringColumna(@w_parametros, '|', 11),
   ...
   Select @w_html = Replace(@w_html,     '&saludos&',       @w_saludo),
          ...
          @w_html = Replace(@w_html,     '&query%',         @w_query),
          @w_html = Replace(@w_html,     '&objeto%',        @w_objeto),
...
Declare
   @w_valor          Varchar(1500) = 'Proceso de Generación de Notificaciones por Incidencias de Tiempos de Proceso.',
   @w_procedimiento  Varchar( 100) = 'Spp_reportaIncTiempoProc'
